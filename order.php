<?php   										// Opening PHP tag
	 ...
	 */
	 ...
	 function get_order_info(PDO $pdo, string $orderNum, string $userEmail) {

		// SQL query to retrieve toy information based on the ordernum and user email
		$sql = "SELECT * 
			FROM orders as o
			join customer as c
			on o.custnum = c.custnum
			WHERE ordernum= :id
			AND email = :userEmail;";	// :id is a placeholder for value provided later 
		                               // It's a parameterized query that helps prevent SQL injection attacks and ensures safer interaction with the database.


		// Execute the SQL query using the pdo function and fetch the result
		$orderInfo = pdo($pdo, $sql, ['id' => $orderNum, 'userEmail' => $userEmail])->fetch();		// Associative array where 'id' is the key and $id is the value. Used to bind the value of $id to the placeholder :id in  SQL query.
	...
	if ($_SERVER["REQUEST_METHOD"] == "POST") {
		 ...
		 */
		$orderInfo = get_order_info($pdo, $orderNum, $email);
